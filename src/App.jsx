...
function App() {
  ...
  const handleUpload = async (acceptedFiles) => {
    ...
    for (const fileObj of newFiles) {
        try {
            const { students, subjects } = await parseCSV(fileObj.originalFile, config)
            ...
            toast.error(`Error processing ${fileObj.name}`, { description: e.message })
        }
    }
  ...
  const handleUrlImport = async (url) => {
      setIsProcessing(true)
      const toastId = toast.loading("Fetching Google Sheet...", { description: "This may take a moment" })
  ...
  const handleDownload = async (file) => {
    ...
    try {
        const blob = await generateWordDocument(file.data.students, file.data.subjects, file.name, config)
  ...
  const handleBulkDownload = async () => {
    const filesToDownload = files.filter(f => selectedFiles.includes(f.id) && f.status === 'success')
